...
public class RepairSchedule {
    ...
    public ArrayList<Integer> availableMechanics() {
        ...
        ArrayList<Integer> availableMechanicsList = new ArrayList<Integer>();
        ArrayList<Integer> allMechanicsList = new ArrayList<Integer>();
        for(int i = 0; i < numberOfMechanics; i++) {
            allMechanicsList.add(i);
        }
        for(Integer num : allMechanicsList) {
            boolean busy = false;
            for(CarRepair repair : schedule) {
                if (repair.getMechanicNum() == num) {
                    busy = true;
                }
            }
            if (!busy) {
                availableMechanicsList.add(num);
            }
        }
